...
//根据ngx_http_upstream_conf_t配置项结构体 启动upstream
...
ngx_http_upstream_init(ngx_http_request_t *r)
{
	...
	//一旦启动upstream机制，就不应该对客户端的读操作带有超时时间的处理，
...
//调用用户注册的请求构建函数(用户自定义)去构建发往上游服务器的请求，同时将建立与上游服务器的连接
...
ngx_http_upstream_init_request(ngx_http_request_t *r)
{
	...
	//检查ngx_http_upstream_conf_t配置结构中的ignore_client_abort标志位
	//该标志位为1时表示就算下游客户端主动关闭了连接，也不会中断与上游服务器之间的交互
    if (!u->store && !r->post_action && !u->conf->ignore_client_abort) {

		//nginx与客户端之间的TCP连接的检查方法
		//实际上这两个方法都会调用ngx_http_upstream_check_broken_connection方法来完成检查
		//如果检查到Nginx与下游的连接是否正常，如果出现错误就会立即终止连接
	...
	//调用ngx_http_upstream_t结构体里由某个HTTP模块实现的create_request方法
	//构造发往上游服务器的请求( u->request_bufs = r->request_body->bufs )
...
ngx_http_upstream_cache_send(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
	...
	//同时需要把包头中的信息设置到headers_in结构体中，
	//之后会把headers_in中设置的头部添加到要发送到下游客户端的响应头部headers_out中
    u->headers_in.content_length_n = -1;
...
/*
*ngx_http_upstream_connect函数中设置的回调函数，
*
*该函数作为nginx与上游服务器的连接的读写事件的回调函数
*
...
/*
* Nginx使用非阻塞套接字与上游服务器建立连接
* 如果连接没有立刻返回成功(如等待V上游服务器返回TCP的SYN/ACK包)，
* 那么需要在epoll中监控这个套接字，当它出现可写事件时，就说明连接已经建立成功了
*/
...
ngx_http_upstream_connect(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
	 ...
	 *调用ngx_socket方法,得到一个socket并设置为非阻塞
	 *调用ngx_get_connection方法，从连接池中获取一个空闲连接，
	 *用来承载Nginx与上游服务器的TCP连接
	 *调用ngx_add_conn方法将TCP套接字以期待可读可写的方法添加到epoll中
	 *调用connect方法向上游服务器发起连接
	 */
	...
	*/
	...
	//将客户端与上游服务器的连接的读写事件的处理回调设置为ngx_http_upstream_handler
	...
	//与上游连接尚未建立起来(rc == NGX_AGAIN,rc是ngx_event_connect_peer的返回值)，则将连接的写事件加入定时器并返回
  	//当与上游服务器连接建立成功后会回调ngx_http_upstream_send_request ( 事件可写 )
    if (rc == NGX_AGAIN) {
...
ngx_http_upstream_send_request(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
	...
	//发送缓冲区中的请求内容到上游服务器,即ngx_http_upstream_t结构体中的request_bufs链表
	//只有第一次调用该方法时需要传送request_bufs
    rc = ngx_output_chain(&u->output, u->request_sent ? NULL : u->request_bufs);

	//表示已经传送了request_bufs缓冲区
    u->request_sent = 1;
    ...
    if (rc == NGX_AGAIN) {
		...
		//将写事件添加到epoll中，ngx_http_upstream_send_request方法结束
	...
	//防止可写事件再次触发而调用ngx_http_upstream_send_request方法
...
//请求的大小未知，需要多次调用该方法才能发完请求的全部内容
//该方法更多的在检测请求的状态，真正发送请求的是ngx_http_upstream_send_request方法
static void
    ...
    ngx_http_upstream_t *u)
{
	...
	//header_sent标志位为1时，表明上游服务器的响应需要直接转发给客户端，而且此时nginx已经把响应包头转发给客户端了
	//实际上这个时候请求已经全部发完了，所以会直接return，不再调用ngx_http_upstream_send_request
...
ngx_http_upstream_process_header(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
	...
	//接收响应头部的buffer缓冲区，u->buffer.start == NULL说明缓冲区还未分配好
    if (u->buffer.start == NULL) {
		//以配置项buffer_size大小来分配缓冲区
    ...
    if (u->headers_in.status_n > NGX_HTTP_SPECIAL_RESPONSE) {
		...
		//subrequest_in_memory表示不转发响应到下游客户端
	...
	//处理已经解析出的响应头部，将已经解析出的头部设置到ngx_http_request_t的headers_out成员中
    if (ngx_http_upstream_process_headers(r, u) != NGX_OK) {
        return;
    }
	...
	//表示再有上游服务器发来响应包体，直接由该方法处理(读事件的回调方法)
...
//process_header的重要职责是将 后端服务器返回的状态翻译成返回给客户端的状态
static ngx_int_t
...
//upstream机制不转发响应时，作为读事件的回调方法在内存中处理上游服务器响应包体
//每次与上游的TCP连接上有读事件触发时，都会调用该方法
//具体的包体处理方法，是调用HTTP模块实现input_filter方法
static void
...
//转发响应包头(不管buffering标志位是否为0，都调用该方法)，不反复调用
//1. 解析出包头转发给客户端
//2. 设置转发包体的方法
//process_header(headers_in(ngx_http_request_t结构体))-->ngx_http_upstream_process_headers(headers_out)-->ngx_http_send_header
static void
ngx_http_upstream_send_response(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    if (!u->buffering) {
        ...
        if (n) {
			...
			//buffer缓冲区清空
			//pos指针一般指向未经处理的响应，last指针一般指向刚接收到的响应
			//全部设为缓冲区起始地址的start指针，即表示清空缓冲区
	...
	//ngx_event_pipe_t结构体的初始化(不是创建，这里仅仅是初始化部分成员)
	//设置向下游转发响应的回调方法
	...
	//存放上游响应时的临时文件，最大长度由max_temp_file_size成员限制
	...
	//初始化预读(在读取包头的时候也预先读取到了部分包体)缓冲区链表
	//该链表中的缓冲区不会分配内存来存放上游响应内容
	//仅使用ngx_buf_t结构体指针指向实际的存放响应包体的内存(&u->buffer)
	...
	//设置处理上下游读写事件的回调函数
	//类似于ngx_http_upstream_process_non_buffered_upstream和ngx_http_upstream_process_non_downstream
    ...
    ngx_http_upstream_t *u)
{
	...
	//这个方法才是真正决定以固定内存块作为缓存时如何转发响应的，注意第二个参数是0
    ...
    ngx_uint_t do_write)
{
	...
	//do_write表示本次是否向下游发送响应
	//lenth=0表示不再需要接收上游的响应，那只能向下游发送响应(即使do_write=0)
    ...
    for ( ;; ) {  //表示与上下游的通信反复执行
        ...
        if (do_write) {
			...
			//out_bufs链表(ngx_buf_t)指向的内容即是buffer中接收到的响应
            if (u->out_bufs || u->busy_bufs) {
				...
				//1. 清空out_bufs链表
				//2. 把out_bufs中已经发送完的ngx_buf_t结构体清空重置--
				//   即把pos/last成员指向start,同时把它们追加到free_bufs链表中
				//3. 如果out_bufs中还有未发送完的ngx_buf_t结构体，那么追加到busy_bufs链表中
...
//当HTTP模块没有实现input_filter处理响应body的方法时，就会默认调用这个方法
static ngx_int_t
ngx_http_upstream_non_buffered_filter(void *data, ssize_t bytes)
{
    ...
    *ll = cl;
	...
	//b即是本次接收到的上游响应包体
	//last成员指向本次接收到的包体的起始地址.
	//ngx_http_upstream_non_buffered_filter的第二个参数bytes表明了本次接收到的包体的字节数
	//剩下的工作就是HTTP模块处理接收到的包体
...
//创建ngx_http_upstream_srv_conf_t
